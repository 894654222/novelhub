...
async def list_changes(
    ...
    """List pending source changes. Admins see all, users see their own."""
    ...
    if user.role in ("admin", "super_admin"):
        if payload.action == "create" and payload.source_data:
            ...
            if source:
                ...
                await db.commit()
        # Return a synthetic change record
        change = SourceChange(
            id=str(uuid4()),
            user_id=user.id,
...
async def review_change(
    ...
    """Approve or reject a pending source change."""
