...
def upgrade() -> None:
    ...
    # Source changes table for approval workflow
    op.create_table(
        'source_changes',
        sa.Column('id', sa.String(), nullable=False),
        sa.Column('user_id', sa.String(), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('action', sa.String(16), nullable=False),  # 'create' or 'delete'
        sa.Column('source_id', sa.String(), nullable=True),  # target source for delete
        sa.Column('source_data', JSONB, nullable=True),  # {id, name, url, plugin_name, config} for create
